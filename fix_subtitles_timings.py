...
Description: Offsets the timings in mis-aligned subrip subtitles files 
License: BSD License
"""
...
def main():
    ...
    # We'll just append a ".offset" to the file we'll write. We want to make sure 
    # it doesn't already exist, to prevent accidental clobbering
    new_filename = subtitles_file+".offset"
    if os.path.isfile(new_filename):
        answer = raw_input("File '%s' already exists. Do you wish to overwrite it?" % new_filename)
        if answer.lower()[0]!="y":
            print "Exiting..."
            sys.exit(1)
    ...
    prog = re.compile(r"(\d{2}):(\d{2}):(\d{2}),(\d{3})[ ]+[-]+>[ ]+(\d{2}):(\d{2}):(\d{2}),(\d{3})")
    ...
    try:        
        for line in open(subtitles_file,'r'):
            ...
            if(re_search):
                ...
                t1 = timedelta(hours=int(h1), minutes=int(m1), seconds=int(s1), milliseconds=int(ms1))
                t2 = timedelta(hours=int(h2), minutes=int(m2), seconds=int(s2), milliseconds=int(ms2))
                # ... add add the offset
                new_t1, new_t2 = t1+offset, t2+offset  
                # Extract the values again (would be great if timedelta had hours and minutes attributes)
                ...
                new_file.write("%02d:%02d:%02d,%03d --> %02d:%02d:%02d,%03d\n" %\
                                (new_h1, new_m1, new_s1, new_ms1,   new_h2, new_m2, new_s2, new_ms2))
            else:
